...
`grok-imagine-image` 属于 Console image 通道，`/v1/images/generations` 和 `/v1/images/edits` 会固定调用 Console 原生路径并透传 native body；Console 失败时直接暴露为上游错误，不再回退到 grok.com legacy 图片服务。
