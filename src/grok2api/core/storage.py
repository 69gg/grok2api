...
# 数据目录（支持通过环境变量覆盖）
# 配置文件路径（保留接口兼容；运行配置使用根目录 config.toml）
...
class SQLStorage(BaseStorage):
    ...
    async def _ensure_schema(self):
        ...
        try:
            ...
            await self._backfill_token_rows()
    ...
    async def _backfill_token_rows(self):
        """将 data JSON 回填到平铺字段"""
        from sqlalchemy import text
        ...
        try:
            ...
            logger.warning(f"SQLStorage: Token row backfill failed: {e}")
...
class StorageFactory:
    ...
    def get_storage(cls) -> BaseStorage:
        ...
        storage_type = (
            ...
            or str(get_config("storage.type", "sqlite") or "sqlite")
        ).lower()
        storage_url = os.getenv("SERVER_STORAGE_URL") or str(
            get_config("storage.url", "") or ""
        )
        ...
        elif storage_type in ("sqlite", "mysql", "pgsql"):
            if storage_type == "sqlite" and not storage_url:
                DATA_DIR.mkdir(parents=True, exist_ok=True)
                storage_url = f"sqlite+aiosqlite:///{DATA_DIR / 'grok2api.db'}"
            if not storage_url:
