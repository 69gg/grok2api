...
from grok2api.core.config import config, get_config
...
def get_admin_api_keys() -> list[str]:
    """返回当前配置的 API Key 列表。"""
    return _normalize_api_keys(get_admin_api_key())
...
def is_valid_admin_api_key(credentials: Optional[str]) -> bool:
    """检查提供的凭证是否匹配任一已配置 API Key。"""
    if not isinstance(credentials, str) or not credentials:
        return False
    for key in get_admin_api_keys():
        if hmac.compare_digest(credentials, key):
            return True
    return False
...
async def verify_api_key(
    ...
    # 标准 api_key 验证
    if is_valid_admin_api_key(auth.credentials):
