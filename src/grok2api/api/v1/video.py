...
from grok2api.services.grok.services.video_extend import VideoExtendService
...
class VideoCreateRequest(BaseModel):
    ...
    image_reference: Optional[Any] = Field(
        None,
        description=(
            "Image references using chat/completions content-block arrays, URL arrays, "
            "or legacy single object/string forms"
        ),
    )
...
def _parse_image_reference_item(value: Any, idx: Optional[int] = None) -> str:
    ...
    if not isinstance(value, dict):
        raise ValidationException(
            message=(
                f"{param_prefix} must be a URL string, "
                f'{{"type": "image_url", "image_url": {{"url": "..."}}}}, '
                "or a legacy single object with image_url"
            ),
            param=param_prefix,
            code="invalid_reference",
        )
    ...
    if "type" in value or isinstance(value.get("image_url"), dict):
    ...
    if has_image_url == has_file_id:
        raise ValidationException(
            message=f"{param_prefix} requires exactly one of image_url or file_id",
            param=param_prefix,
            code="invalid_reference",
        )
    ...
    if has_file_id:
        raise ValidationException(
            message=(
                f"{param_prefix}.file_id is not supported in current reverse pipeline; "
                "please use image_url or multipart input_reference"
            ),
            param=f"{param_prefix}.file_id",
            code="unsupported_reference",
        )

    return _validate_reference_value(image_url, f"{param_prefix}.image_url")
...
def _parse_image_references(value: Any) -> List[str]:
    ...
    if isinstance(value, str):
        ...
        if stripped[0] in {"{", "["}:
            try:
                ...
                # Keep legacy plain url/data-uri fallback for multipart text fields.
    ...
    if isinstance(value, list):
        if not value:
            return []
        return [
            _parse_image_reference_item(item, idx=i)
            for i, item in enumerate(value)
        ]

    if isinstance(value, dict):
        return [_parse_image_reference_item(value)]

    raise ValidationException(
        message="image_reference must be a string, object, or array",
...
def _multipart_create_schema(default_seconds: int) -> Dict[str, Any]:
    return {
        ...
        "properties": {
            ...
            "image_reference": {
                "type": "string",
                "description": "JSON string for image_reference array/object, or a plain URL/data URI string",
            },
