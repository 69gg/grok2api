...
from grok2api.core.exceptions import UpstreamException, AppException, ValidationException, ErrorType
...
from grok2api.services.grok.model import ModelService
from grok2api.services.token import get_token_manager
from grok2api.services.grok.processor import VideoStreamProcessor, VideoCollectProcessor
...
class VideoService:
    ...
    async def generate(
        ...
        async with _get_media_semaphore():
            ...
            try:
                ...
                session = AsyncSession(impersonate=get_impersonate())
                response = await session.post(
                    CHAT_API,
                    headers=headers,
    ...
    async def completions(
        ...
        # 提取内容
        from grok2api.services.grok.chat import MessageExtractor
