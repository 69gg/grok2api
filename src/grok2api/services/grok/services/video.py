...
from grok2api.services.grok.utils.download import DownloadService
from grok2api.services.grok.utils.errors import no_token_error
from grok2api.services.grok.utils.process import _is_http2_error, _normalize_line, _with_idle_timeout
from grok2api.services.grok.utils.retry import rate_limited
...
from grok2api.services.reverse.media_post import MediaPostReverse
from grok2api.services.reverse.media_post_link import MediaPostLinkReverse
from grok2api.services.reverse.utils.session import ResettableSession
from grok2api.services.reverse.video_upscale import VideoUpscaleReverse
from grok2api.services.token import EffortType, get_token_manager
from grok2api.services.token.manager import BASIC_POOL_NAME
...
def _extract_upstream_status(error: Exception) -> Optional[int]:
    if isinstance(error, UpstreamException):
        if isinstance(error.details, dict) and "status" in error.details:
...
def _exclude_token_variants(excluded_tokens: set[str], token_value: str) -> None:
    token_text = str(token_value or "").strip()
    if not token_text:
        return
    excluded_tokens.add(token_text)
    if token_text.startswith("sso="):
        excluded_tokens.add(token_text[4:])
    else:
        excluded_tokens.add(f"sso={token_text}")


def _token_hint(token_value: str) -> str:
    token_text = str(token_value or "").strip()
    if token_text.startswith("sso="):
        token_text = token_text[4:]
    if not token_text:
        return "empty"
    return hashlib.sha1(token_text.encode("utf-8")).hexdigest()[:10]


def _upstream_error_summary(error: Exception) -> Dict[str, Any]:
    if not isinstance(error, UpstreamException):
        return {"error": str(error)}
    details = error.details or {}
    body = str(details.get("body") or details.get("error") or "")
    return {
        "status": details.get("status"),
        "contentType": details.get("content_type"),
        "retryAfter": details.get("retry_after"),
        "requestId": details.get("request_id"),
        "proxyKey": details.get("proxy_key"),
        "proxyTarget": details.get("proxy_target"),
        "referer": details.get("referer"),
        "tokenHint": details.get("token_hint"),
        "payload": details.get("payload") or details.get("payload_summary"),
        "bodyPreview": body[:400],
    }


def _raise_video_rate_limited(last_error: Optional[Exception] = None) -> None:
    if last_error and not rate_limited(last_error):
        raise last_error
    raise AppException(
        message="All available video tokens are rate limited (upstream 429). Please try again later.",
...
def _to_upstream_video_resolution(resolution_name: str) -> str:
    return "HD" if str(resolution_name or "").strip() == "720p" else "SD"
    ...
    return {
        "modelMap": {
            "videoGenModelConfig": {
                ...
                "videoResolution": _to_upstream_video_resolution(resolution_name),
            }
        }
    }
...
class VideoService:
    ...
    async def completions(
        ...
        from grok2api.services.grok.services.chat import MessageExtractor
        ...
        for attempt in range(max_token_retries):
            selected_pool_name: Optional[str] = None
            last_seed_error: Optional[UpstreamException] = None
            token = ""
            raw_token = ""
            ...
            while True:
                seed_pool_candidates = (
                    [selected_pool_name] if selected_pool_name else pool_candidates
                )
                token_info = token_mgr.get_token_for_video(
                    resolution=resolution,
                    video_length=video_length,
                    pool_candidates=seed_pool_candidates,
                    exclude_tokens=excluded_tokens,
                )

                if not token_info:
                    if last_error and rate_limited(last_error):
                        _raise_video_rate_limited(last_error)
                    if last_seed_error and rate_limited(last_seed_error):
                        _raise_video_rate_limited(last_seed_error)
                    if last_error:
                        raise last_error
                    if last_seed_error:
                ...
                token = raw_token[4:] if raw_token.startswith("sso=") else raw_token
                pool_name = token_mgr.get_pool_name_for_token(token) or BASIC_POOL_NAME
                if selected_pool_name is None:
                    selected_pool_name = pool_name
                logger.info(
                    "Video seed selection: attempt={}/{}, pool={}, token_hint={}, selected_pool={}, excluded_tokens={}, resolution={}, video_length={}, has_image_reference={}",
                    attempt + 1,
                    max_token_retries,
                    pool_name,
                ...
                try:
                    ...
                    if status == 403 and selected_pool_name == pool_name:
                        _exclude_token_variants(excluded_tokens, raw_token)
                        await token_mgr.record_fail(
                            token,
                            status,
                            "video_media_post_forbidden",
                        )
                        last_seed_error = e
                        last_error = e
                        logger.warning(
                            "Video seed media post forbidden for token_hint={} in pool={}, retrying with another token in the same pool, upstream={}",
                            _token_hint(raw_token),
                            pool_name,
                            _upstream_error_summary(e),
                        )
                        continue
                    if rate_limited(e):
                        _exclude_token_variants(excluded_tokens, raw_token)
                        ...
                        last_error = e
                        logger.warning(
                            "Video seed media post rate limited for token_hint={} in pool={}, trying next token (attempt {}/{}), upstream={}",
                            _token_hint(raw_token),
                            pool_name,
                            attempt + 1,
                            max_token_retries,
            ...
            try:
                ...
                if rate_limited(e):
                    ...
                    last_error = e
                    logger.warning(
                        "Video round request rate limited for token_hint={} in pool={}, retrying whole generation with another token (attempt {}/{}), upstream={}",
                        _token_hint(raw_token),
                        pool_name,
                        attempt + 1,
                        max_token_retries,
