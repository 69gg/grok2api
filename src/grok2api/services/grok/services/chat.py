...
from grok2api.services.grok.utils.usage import estimate_chat_usage, estimate_prompt_tokens
from grok2api.services.token import get_token_manager, EffortType
...
def _split_reasoning_sections(text: str) -> tuple[str, str]:
    """Extract legacy <think> blocks from a text payload."""
    ...
    while cursor < len(text):
        ...
        end = text.find("</think>", body_start)
        if end == -1:
...
class ChatService:
    ...
    async def completions(
        ...
        emit_legacy_think_tags: bool = False,
    ):
        """Chat Completions 入口"""
        # 获取 token
...
class StreamProcessor(proc_base.BaseProcessor):
    ...
    def __init__(
        ...
        self.include_internal_reasoning = bool(include_internal_reasoning)
        self.tools = tools
        self.tool_choice = tool_choice
        self._tool_stream_enabled = bool(tools) and tool_choice != "none"
        self._tool_parser = (
            ToolCallStreamParser(self.tools) if self._tool_stream_enabled else None
        )
        ...
        self._tool_calls_output: list[dict[str, Any]] = []
    ...
    def _handle_tool_stream(self, chunk: str) -> list[tuple[str, Any]]:
        if not chunk:
            return []
        if not self._tool_stream_enabled or not self._tool_parser:
            return [("text", chunk)]

        events: list[tuple[str, Any]] = []
        allow_calls = not self._content_emitted and not self._tool_calls_seen
        for kind, payload in self._tool_parser.feed(chunk, allow_calls=allow_calls):
            if kind == "tool":
                indexed_tool_call = self._with_tool_index(payload)
                self._tool_calls_output.append(indexed_tool_call)
                events.append(("tool", indexed_tool_call))
                self._tool_calls_seen = True
            else:
                if self._tool_calls_seen and isinstance(payload, str) and payload.strip():
                    continue
    ...
    def _flush_tool_stream(self) -> list[tuple[str, Any]]:
        ...
        events: list[tuple[str, Any]] = []
        for kind, payload in self._tool_parser.flush():
            if kind == "tool":
                indexed_tool_call = self._with_tool_index(payload)
                self._tool_calls_output.append(indexed_tool_call)
                events.append(("tool", indexed_tool_call))
    ...
    def _sse(
        ...
        internal_reasoning_content: str | None = None,
    ) -> str:
        """Build SSE response."""
        delta = {}
        if role:
        ...
        elif internal_reasoning_content:
            delta["_grok2api_reasoning"] = internal_reasoning_content
        elif content:
        ...
        try:
            async for line in proc_base._with_idle_timeout(
                ...
                if img := resp.get("streamingImageGenerationResponse"):
                    ...
                    reasoning_chunk = f"正在生成第{idx}张图片中，当前进度{progress}%\n"
                    self._reasoning_parts.append(reasoning_chunk)
                    if self.include_internal_reasoning:
                        yield self._sse(internal_reasoning_content=reasoning_chunk)
                    elif self.show_think:
                        if not self.think_opened:
                            yield self._sse("<think>\n")
                            self.think_opened = True
                        yield self._sse(reasoning_chunk)
                    continue
                ...
                if (token := resp.get("token")) is not None:
                    ...
                    if (
                        ...
                        and not self.include_internal_reasoning
                    ):
                        continue
                    ...
                    in_think = (
                        (
                            is_thinking
                            and (
                                self.include_internal_reasoning
                                or not self.think_closed_once
                            )
                        )
                        or self.image_think_active
                    )
                    if in_think:
                        self._reasoning_parts.append(filtered)
                        if self.include_internal_reasoning:
                            yield self._sse(internal_reasoning_content=filtered)
                            continue
                    ...
                    if self._tool_stream_enabled:
                        for kind, payload in self._handle_tool_stream(filtered):
                            if kind == "text":
                                self._output_parts.append(payload)
                                yield self._sse(payload)
                            elif kind == "tool":
                                yield self._sse(tool_calls=[payload])
                        continue

                    self._output_parts.append(filtered)
                    yield self._sse(filtered)
            ...
            if self._tool_stream_enabled:
                ...
                yield self._sse(
                    ...
                    usage=estimate_chat_usage(
                        ...
                        tool_calls=self._tool_calls_output,
...
class CollectProcessor(proc_base.BaseProcessor):
    ...
    async def process(self, response: AsyncIterable[bytes]) -> dict[str, Any]:
        ...
        content = self._filter_content("".join(streamed_text_parts) or content)
        reasoning_text = "".join(reasoning_parts).strip() or None
        if not reasoning_text and content and "<think>" in content:
            extracted_reasoning, visible_text = _split_reasoning_sections(content)
            if extracted_reasoning.strip():
                reasoning_text = extracted_reasoning.strip() or None
                content = visible_text.strip() or None
        ...
        if self.include_internal_reasoning and reasoning_text:
            message_obj["_grok2api_reasoning_text"] = reasoning_text
