...
from grok2api.core.storage import get_storage, StorageError
...
from grok2api.services.reverse.utils.retry import RetryContext, extract_retry_after
...
class TokenManager:
    ...
    def get_token_for_video(
        ...
        for idx, pool_name in enumerate(ordered_pools):
            token_info = self.get_token_info(pool_name, exclude=exclude_tokens)
            if token_info:
                if idx == 0:
                    logger.info(
