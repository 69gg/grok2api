...
def is_cloudflare_challenge(error: Exception) -> bool:
    """Best-effort detect Cloudflare browser challenge responses."""
    if not isinstance(error, UpstreamException):
        return False
    details = error.details or {}
    if details.get("is_cloudflare_challenge") is True:
        return True
    ...
    content_type = str(
        details.get("content_type") or headers.get("content-type") or ""
    ).lower()
    ...
    if "cloudflare" in server and "text/html" in content_type:
        lowered = body.lower()
        if "just a moment" in lowered or "challenge-platform" in lowered:
            return True
    return False
    ...
    def should_retry(self, status_code: int, error: Exception = None) -> bool:
        ...
        if isinstance(error, UpstreamException) and error.details:
            ...
            if is_cloudflare_challenge(error):
                logger.warning("Detected Cloudflare challenge, skipping retries.")
                return False
        # --- 准确判定逻辑结束 ---
