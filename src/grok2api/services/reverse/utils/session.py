...
from grok2api.services.reverse.utils.headers import resolve_proxy_browser
...
class ResettableSession:
    ...
    def __init__(
        ...
        **session_kwargs: Any,
        ...
        configured_browser = self._session_kwargs.get("impersonate") or get_config("proxy.browser")
        resolved_browser = resolve_proxy_browser(
            configured_browser,
            get_config("proxy.user_agent"),
        )
        if resolved_browser:
            self._session_kwargs["impersonate"] = resolved_browser
    ...
    async def _request(self, method: str, *args: Any, **kwargs: Any):
        ...
        requested_browser = kwargs.get("impersonate") or self._session_kwargs.get("impersonate")
        resolved_browser = resolve_proxy_browser(
            requested_browser,
            get_config("proxy.user_agent"),
        )
        if resolved_browser:
            kwargs["impersonate"] = resolved_browser
