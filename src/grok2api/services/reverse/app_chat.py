...
def _is_video_gen_request(
    tool_overrides: Optional[Dict[str, Any]],
    model_config_override: Optional[Dict[str, Any]],
) -> bool:
    if isinstance(tool_overrides, dict) and bool(tool_overrides.get("videoGen")):
        return True
    if not isinstance(model_config_override, dict):
        return False
    model_map = model_config_override.get("modelMap")
    if not isinstance(model_map, dict):
        return False
    return isinstance(model_map.get("videoGenModelConfig"), dict)
...
class AppChatReverse:
    ...
    def build_payload(
        ...
        is_video_request = _is_video_gen_request(tool_overrides, model_config_override)
    ...
    async def request(
        ...
        try:
            ...
            # Build headers
            is_video_request = _is_video_gen_request(tool_overrides, model_config_override)
            referer = "https://grok.com/imagine" if is_video_request else "https://grok.com/"
            ...
            async def _do_request():
                nonlocal active_proxy_key, active_proxy_target
                active_proxy_key, base_proxy = get_current_proxy_from("proxy.base_proxy_url")
                proxy = None
                proxies = None
                if base_proxy:
                    normalized_proxy = _normalize_chat_proxy(base_proxy)
                    active_proxy_target = normalized_proxy
                    scheme = urlparse(normalized_proxy).scheme.lower()
                    if scheme.startswith("socks"):
                        # curl_cffi 对 SOCKS 代理优先使用 proxy 参数，避免被按 HTTP CONNECT 处理
                        proxy = normalized_proxy
                ...
                if response.status_code != 200:
                    ...
                    cf_mitigated = str(response_headers.get("cf-mitigated", "")).lower()
                    server_header = str(response_headers.get("server", "")).lower()
                    ...
                    logger.error(
                        "AppChatReverse: Chat failed, status={}, content_type={}, retry_after={}, is_cloudflare_challenge={}, headers={}, body={}",
                    ...
                    raise UpstreamException(
                        ...
                        details={
                            ...
                            "proxy_target": active_proxy_target,
                            "referer": referer,
